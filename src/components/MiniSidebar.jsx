    ...
    <aside className="w-10 float-right" aria-label="Sidebar">
      <div className="overflow-y-auto rounded-tl-md rounded-bl-md mt-20 ">
        <ul className="">
          <li
            onClick={() => {
              dispatch(switchNum(0));
              dispatch(openDrawer(true));
            }}
            className={`h-10 w-10 p-2 ${
              tab === 0 ? "bg-[#11366b54]" : "bg-white"
            } `}
          >
            <RobotIcon />
          </li>
          <li
            onClick={() => dispatch(switchNum(1))}
            className={`h-10 w-10 p-2 ${
              tab === 1 ? "bg-[#11366b54]" : "bg-white"
            } `}
          >
            <MessageIcon />
          </li>
          <li
            onClick={() => dispatch(switchNum(2))}
            className={`h-10 w-10 p-2 ${
              tab === 2 ? "bg-[#11366b54]" : "bg-white"
            } `}
          >
            <CircleCheckIcon />
          </li>
          <li
            onClick={() => dispatch(switchNum(3))}
            className={`h-10 w-10 p-2 ${
              tab === 3 ? "bg-[#11366b54]" : "bg-white"
            } `}
          >
            <BooksIcon />
          </li>
